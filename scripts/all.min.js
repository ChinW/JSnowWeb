document.addEventListener("DOMContentLoaded", function(){
	var transitionLayer = $(".js-transition-layer"),
	...
	function setLayerDimensions(){
		...
		setTimeout(function(){
			transitionLayer.toggleClass("visible opening")
			$("#js-main-content").toggleClass("visible")
			setTimeout(function(){
...
jQuery(document).ready(function($) {
    ...
    $('a[href="#js-product-tour"]').on('click', function(event) {
        event.preventDefault();
        $('header').addClass('slide-down');
        if ($(window).width() < MqL) {
            $('body,html').animate({ 'scrollTop': $('#js-product-tour').offset().top - 30 }, 200);
        } else {
            $('.js-main-content').addClass('is-product-tour');
            requestVideo(jQuery('.js-active'));
        }
    });

    $("a[href='#js-main-content']").click(function(event) {
        if ($(window).width() >= MqL) {
            event.preventDefault()
            showProductIntro()
        }
    ...
    $('.js-prev').on('click', function(event) {
        event.preventDefault();
        var activeSlide = $('.js-active');
        // console.log("in prev click", activeSlide.index())
    ...
    $('.js-next').on('click', function(event) {
        event.preventDefault();
        var activeSlide = $('.js-active');
        // console.log("in next click", activeSlide.index())
        updateSlider(activeSlide, 'next');
    });

    $("#priceBtn").on('click', function(){
        $(".js-product-description").toggleClass("hide");
        $(".js-product-price").toggleClass("show");
    });

    $(document).keyup(function(event) {
        if (event.which == '37' && $('.js-main-content').hasClass('is-product-tour')) {
            var activeSlide = $('.js-active');
            if (activeSlide.is(':first-child')) {
                //in this case - switch from product tour div to product intro div
                showProductIntro();
        ...
        } else if (event.which == '39' && $('.js-main-content').hasClass('is-product-tour')) {
            var activeSlide = $('.js-active');
            updateSlider(activeSlide, 'next');
        }
    });
    ...
    $(window).on('resize', function() {
        window.requestAnimationFrame(function() {
            if ($(window).width() < MqL) {
                ...
                $('.js-single-item.js-active').find('video').show();
                ($('.js-main-content').hasClass('is-product-tour')) ? $('header').addClass('slide-down'): $('header').removeClass('slide-down');
            }
        });
    });
    ...
    $(window).on('scroll', function() {
        window.requestAnimationFrame(function() {
            if ($(window).width() < MqL && $(window).scrollTop() < $('#js-product-tour').offset().top - 30) {
                $('header').removeClass('slide-down');
            } else if ($(window).width() < MqL && $(window).scrollTop() >= $('#js-product-tour').offset().top - 30) {
                $('header').addClass('slide-down');
            }
        });
    });
    ...
    function showProductIntro() {
        $('header').removeClass('slide-down');
        $('.js-main-content').removeClass('is-product-tour');

        $("#js-product-tour ul li").each(function() {
            if ($(this).is(":first-child")) {
                $(this).removeClass("js-hidden js-not-visible").addClass("js-active")
            } else {
                $(this).removeClass("js-active js-hidden").addClass("js-not-visible js-move-right")
            }
        })


        $(".js-next").removeClass("js-inactive")
        $('.js-active').find('video').get(0).pause();
        $('.js-single-item').find('video').each(function() {
    ...
    function updateSlider(active, direction) {
        ...
        if (direction == 'next') {
            ...
            //so we have to change .js-single-item childrens attributes after having changed its visibility value
            setTimeout(function() {
                active.removeClass('js-active').addClass('js-hidden').next().removeClass('js-move-right').addClass('js-active')
                active.next().one('transitionend', function() {
                    // console.log(active.index())
                    active.addClass('js-not-visible');
                })
            }, 50);

            // setTimeout(function() {
            //    	active.removeClass('js-active').addClass('js-hidden').next().removeClass('js-move-right').addClass('js-active').one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend', function(){
            //    		console.log(active.index())
            //    		active.addClass('js-not-visible');
            //    	});
            // }, 50);
            ...
            //on Firefox CSS transition/animation fails when parent element changes visibility attribute
            //so we have to change .js-single-item childrens attributes after having changed its visibility value
            active.removeClass('js-active').addClass('js-move-right').prev().addClass('js-active')
            active.prev().one('transitionend', function() {
                    // console.log(active.index())
                    active.addClass('js-not-visible');
                })
                // setTimeout(function() {
                //    	active.removeClass('js-active').addClass('js-move-right').prev().addClass('js-active').one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend', function(){
                //    		console.log(active.index())
                //    		active.addClass('js-not-visible');
                //    	});
                // }, 50);
        ...
        selected.removeClass('js-not-visible js-hidden');
        //update slider navigation (in case we reached the last slider)
        updateSliderNav(selected);
        //load the video for the new slider
        requestVideo(selected);

    }

    function updateSliderNav(selected) {
        (selected.is(':last-child')) ? $('.js-next').addClass('js-inactive'): $('.js-next').removeClass('js-inactive');
        $('.js-loader').stop().hide().css('width', 0);
    }

    function requestVideo(selected) {
        return 1
        selected.siblings('.js-single-item').find('video').each(function() {
        ...
        if (selected.find('video').length > 0) {
            ...
            var videoUrl = "http://7xngrd.com1.z0.glb.clouddn.com/js_" + selected.find('.js-image-container img').data('video'),
                video = $('<video loop><source src="' + videoUrl + '.mp4" type="video/mp4" />Sorry, your browser does not support HTML5 video.</video>');
            video.appendTo(selected.find('.js-image-wrapper')).hide();
            ...
            loader.show().animate({ width: '50%' }, 1500, function() {
                var timeout = setInterval(function() {
                    if (loaded) {
                        //this means the video is ready - complete .js-loader and play the video
                        loader.animate({ width: '100%' }, 100, function() {
                            loader.css('width', 0);
                        ...
                        var windowWidth = $(window).width(),
                            widthNew = loader.width() + 10;
                        if (widthNew < windowWidth) {
                            loader.show().animate({ width: widthNew + 'px' }, 500);
                        }
                    }
                }, 500);
            });
    ...
    //     data: {"aId": "b8068cb246bb58757df024f2b6f93035", "_api_key": "de9203df32e7b1914e9a74ba017d8ff2"},
    //     type: "post",
    //     dataType: "jsonp",
        // success: function(data) {
            // console.log(data)
            // $("#android-downloader-button").attr("href", data.installUrl);
            // $("#android-downloader-version").html(data.versionShort);
    //     }
    // })

    $.ajax({
        url: "http://api.fir.im/apps/latest/57a8b34d959d6914280000b1",
        data: { api_token: "a04029a2a29c569cfac2b4be6fc83228" },
        success: function(data) {
            // $("#android-downloader-button").attr("href", data.installUrl);
            $("#android-downloader-version").html(data.versionShort);
        }
    ...
    $.ajax({
      url: "https://itunes.apple.com/lookup?id=1061937942",
      dataType: "jsonp",
      success: function(data) {
            if(data.results.length > 0){
                var obj = data.results[0];
                $("#iOS-downloader-version").html(obj.version);
            }
        }
    });
});
